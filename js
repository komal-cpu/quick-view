...
function quickView() {
  ...
  $(document).on('click','.quick-view', function(){
    ...
    var product_handle = $(this).data('handle');
     product_lang = $(this).data('lang');
    product_lang = (product_lang == 'fr')?'/fr':'';
    $('#quick-view').addClass(product_handle);
    jQuery.getJSON(product_lang+'/products/' + product_handle + '.js', function (product) {
      var id = product.id;
      ...
      var fimage = product.featured_image;
      var variants = product.variants;
      var options = product.options;
      var url = '/products/' + product_handle;
      $('.qv-product-title').text(title);
      $('.qv-product-description').html(desc0);
      $('.qv-product-type').text(type);
      if(desc1){
        $('.qv-product-description1').html(desc1);
        $('.qv-product-description2').html(desc2);}
      else{
        $('.qv-product-description1').remove();
      
      }

        if(desc3){

      $('.qv-product-description3').html(desc3);
         $('.qv-product-description4').html(desc4);}
      else{
         $('.qv-product-description3').remove();
    
      }
      $('.view-product').attr('href', url);
      $(images).each(function (i, image) {
   $('.qv-product-images1').append('<div><img src="' + image + '"></div>');
           });
    ...
    $.fancybox({
      ...
      'afterShow': function () {
        var x = $(".supports-js").attr('lang');
  $('#quick-view').hide().html(content).css('opacity', '1').attr("lang", x).attr("ly-is-observing","true").fadeIn(function () {
      ...
      'afterClose': function () {
        ...
        $(document).on("click", ".qv-add-button", function () {
          ...
          $('#quick-view select').each(function (i) {
            if (selectedOptions == '') {
              selectedOptions = $(".qv-product-title").text();
            } else {
              selectedOptions = selectedOptions + ' / ' + $(this).val();
            }
          });
         ...
         //   alert(product_handle);
            $(product.variants).each(function (i, v) {
             ...
             // alert(v.name);
              // (v.title == selectedOptions)   // old code 
              if (v.name == selectedOptions) {
                var_id = v.id;
               processCart();
                //  alert(id);
              	//alert(v.name+"===="+selectedOptions);
              }
            });
